...
export const AdminGoodsReceiptPdfViewerPage: React.FC = () => {
  const { goodsReceipt, goodsReceiptId } = useOutletContext<AdminGoodsReceiptDetailOutletContext>();
  ...
  useEffect(() => {
    ...
    void downloadGoodsReceiptDocument(goodsReceiptId)
      .then(doc => {
        if (cancelled) return;
  ...
  }, [goodsReceiptId, useNativeViewer]);
  ...
  return (
    <section className="invoice-detail-pdf invoice-detail-pdf--fullscreen panel glass">
      ...
      {loading ? (
        <FetchingLoader label="Loading goods receipt PDF…" />
      ) : error ? (
        ...
        <iframe
          title={`Goods receipt ${goodsReceipt.billNumber}`}
          src={pdfUrl}
