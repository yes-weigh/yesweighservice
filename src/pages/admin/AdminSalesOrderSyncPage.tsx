...
import {
  ...
  zohoApiUsageLabel,
  zohoApiUsageTone,
  type OrgSalesOrderSyncStatus,
  type ZohoApiUsageStatus,
} from '../../lib/org-sales-order-sync';

function formatLastRunSummary(status: OrgSalesOrderSyncStatus | null): string | null {
...
export const AdminSalesOrderSyncPage: React.FC = () => {
  const [status, setStatus] = useState<OrgSalesOrderSyncStatus | null>(null);
  ...
  useCatalogPageHeader({ title: 'Sales order sync' }, true);
  ...
  const loadStatus = useCallback(async (options?: { quiet?: boolean; fromButton?: boolean }) => {
    ...
    if (!quiet) setError('');
    try {
      const next = await fetchOrgSalesOrderSyncStatus();
      setStatus(next);
      if (fromButton) {
        setNotice(
          next.totalInRange == null
            ? 'Status refreshed — Zoho total not counted yet. Use Count sales orders below.'
            : 'Status refreshed.',
        );
        setError('');
      }
  ...
  const handleCount = useCallback(async () => {
    setBusy('count');
    showActionFeedback(
      'Counting sales orders in Zoho… this can take a few minutes. Keep this tab open.',
    );
    try {
      const result = await countOrgSalesOrdersInRange();
      await loadStatus();
  ...
  const handleCategoryBackfill = async () => {
    setBusy('category');
    showActionFeedback(
      'Classifying sales orders from line item → catalog (HSN/category)… no Zoho calls.',
    );
    try {
    ...
    void runOrgSalesOrderSync()
      .then(result => {
        ...
        showActionFeedback(
          result.message
          ?? (result.completed
            ? `Complete — ${parts.join(', ')}. All ${result.pulledCount.toLocaleString()} sales orders are in Firestore.`
  ...
  if (loading && !status) {
    return <FetchingLoader label="Loading sales order sync status…" />;
  }
  ...
  return (
    <div className="page-content fade-in">
      <div className="mb-6">
        <Link to="/super-admin/sales-orders" className="text-muted text-sm admin-invoices-back">
          ← All sales orders
        </Link>
        <h1 className="mt-2">Sales order sync</h1>
        <p className="text-muted mt-2">
          Org-wide backfill from Zoho into Firestore — one document per sales order, details only (no PDFs).
          Customer name and id are stored on each document. A scheduled run at <strong>4:00 AM IST</strong> resumes
          ...
          <div>
            ...
            {total == null && (
              <p className="text-muted text-sm mt-1 mb-0">
                Not counted yet — Refresh status or Count sales orders
              </p>
            )}
          </div>
        </div>
      ...
      <div className="panel glass">
        ...
        <p className="text-muted mb-4">
          <strong>Count sales orders</strong> only refreshes the totals above.
          {' '}
        ...
        <div className="flex gap-3 flex-wrap">
          ...
          >
            ...
            {busy === 'count' ? 'Counting…' : 'Count sales orders'}
          </button>
          <button
            type="button"
