...
import {
  ...
  filterAdminSalesOrdersByPeriod,
...
function SalesOrderFilterSheet({
  open,
  rangePreset,
  category,
  sort,
...
export const AdminSalesOrdersPage: React.FC = () => {
  ...
  const periodRows = useMemo(
    () => filterAdminSalesOrdersByPeriod(rows, rangePreset),
    [rows, rangePreset],
  );

  const filtered = useMemo(
    () => filterAdminSalesOrders(periodRows, search, category),
  ...
  const openPo = (po: AdminFirestoreSalesOrder) => {
    navigate(`${basePath}/sales-orders/${po.id}`);
  };

  const summary = useMemo(() => {
    const salesEntries = buildAdminSalesOrderSalesEntries(filtered);
    const sales = salesEntries.length ? computeSalesForPeriod(salesEntries, rangePreset) : null;
    const categoryAmount = category === 'all'
      ? (sales?.totalSales ?? 0)
      : filtered.reduce((sum, row) => sum + Number(row.categoryAmounts[category] ?? row.total ?? 0), 0);
    return {
        ...
        <div className="invoices-summary__kpis">
          <div className="invoices-summary__kpi">
            ...
            <div className="invoices-summary__kpi-body">
              <span className="invoices-summary__kpi-label">Total SOs</span>
          ...
          <div className="invoices-summary__kpi">
            ...
            <div className="invoices-summary__kpi-body">
              ...
              <strong className="invoices-summary__kpi-value invoices-summary__kpi-value--amount">
                {loading ? '…' : formatCurrency(category === 'all' ? summary.totalAmount : summary.categoryAmount)}
              </strong>
              <span className="invoices-summary__kpi-sub">
            ...
            <div className="panel glass invoices-table-panel admin-invoices-table-panel">
              <div className="invoices-table-wrap invoices-table-wrap--desktop">
                <table className="invoices-table">
                  ...
                  <tbody>
                    {pageRows.map(po => {
                      const categoryLabel = invoiceCategoryLabel(po.salesOrderCategory);
                      return (
                        <tr
                          key={po.id}
